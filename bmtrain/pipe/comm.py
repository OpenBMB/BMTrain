import torch
from ..distributed.ops import send_activations_list, recv_activations_list, send_activations, recv_activations
...
class PipeCommander:
    ...
    def send_backward_recv_forward(self, backward_grad, need_data=False):
        if not self.is_first_stage():
            self.send_pre(backward_grad)
            forward_state = self.recv_pre()
        else:
            if need_data:
