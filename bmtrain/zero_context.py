...
class ZeroContext:
    """ZeroContext designed to reduece the memroy footprint required for large deep learning model.
    For forward, ZeroContetx gathers param buffer before using and releases params after using.
    For backward, ZeroContext gathers param buffer before using and releases params after using, also keep memory for grad and reduce-scatter grad after computing.

    Attributes:
        block (BlocK): Block for using ZeroContext.
        ctx_dict (dict): model layer dict.
        _param_budder (dict): buffer for Block Allgather layer storage, all parameters.
        _grad_buffer (dict): buffer for all gard.
        _param_tensor (dict): tensors reference to _param_buffer.
        _grad_tensor (dict): tensors reference to _grad_buffer.
        _need_release (bool): whether to release buffer.
