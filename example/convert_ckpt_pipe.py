...
def key_process(key, pipe_size , rank, start, end):
    ...
    if layer_idx is None or (layer_idx >= start and layer_idx < end): 
        if rank == 0:
            if key == "word_emb.weight":
                return "transformers.0.weight"
            elif key == "pos_emb.weight":
                return "transformers.1.weight"
            if key.startswith("layernorm"):
                return None
            else:
                if layer_idx is not None:
                    return re.sub(r"\d+", str(layer_idx + 2), key)
        elif rank == pipe_size - 1:
            if key == "word_emb.weight":
                return "transformers.3.weight"
            if key.startswith("layernorm"):
                postfix = key.split(".")[-1]
                return f"transformers.2.{postfix}"
            elif key == "pos_emb.weight":
                return None
            else:
                print(key)
