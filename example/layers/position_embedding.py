...
class PositionEmbedding(bmp.DistributedModule):
    def __init__(self, num_heads, num_buckets, max_distance, bidirectional=True, dtype=torch.half):
        super().__init__()
        self.weight = bmp.DistributedParameter(torch.randn(num_heads, num_buckets, dtype=dtype), init_method=bmp.ParameterInitializer(torch.nn.init.normal_, mean=0.0, std=1))
