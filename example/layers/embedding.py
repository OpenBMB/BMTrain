...
class Embedding(bmp.DistributedModule):
    def __init__(self, vocab_size : int, embedding_size : int, dtype=torch.half):
        super().__init__()
        self.weight = bmp.DistributedParameter(torch.empty(vocab_size, embedding_size, dtype=dtype), init_method=bmp.ParameterInitializer(torch.nn.init.normal_, mean=0.0, std=0.02))
