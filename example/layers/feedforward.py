...
class FeedForward(bmp.DistributedModule):
    def __init__(self, dim_model : int, dim_ff : int, int8=True, dtype=torch.half):
        super().__init__()
        self.w_0 = bmp.DistributedParameter(torch.empty(dim_ff, dim_model, dtype=dtype), init_method=bmp.ParameterInitializer(torch.nn.init.normal_, mean=0.0, std=0.02))
        self.w_1 = bmp.DistributedParameter(torch.empty(dim_ff, dim_model, dtype=dtype), init_method=bmp.ParameterInitializer(torch.nn.init.normal_, mean=0.0, std=0.02))
        self.w_out = bmp.DistributedParameter(torch.empty(dim_model, dim_ff, dtype=dtype), init_method=bmp.ParameterInitializer(torch.nn.init.normal_, mean=0.0, std=0.02))
